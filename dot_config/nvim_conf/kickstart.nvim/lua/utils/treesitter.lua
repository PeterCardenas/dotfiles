...
local function get_ts_info(bufnr, row, col)
  if (row and (not bufnr or row < 0 or row > vim.api.nvim_buf_line_count(bufnr))) or (col and col < 0) then
    return nil, nil, nil
  end
  local success, node_under_cursor = pcall(vim.treesitter.get_node, {
    bufnr = bufnr,
...
function M.inside_comment_block(bufnr, row, col)
  ...
  local success, inside_comment_block = pcall(function()
    for id, node, _ in query:iter_captures(node_under_cursor, bufnr or 0, row, row + 1) do
