...
local function create_harpoon_keymap(mark_no)
  vim.keymap.set('n', string.format('<leader>%d', mark_no), function()
    require('harpoon.ui').nav_file(mark_no)
  end, { desc = string.format('Go to harpoon mark %d', mark_no) })
  vim.keymap.set('n', string.format('<leader>m%d', mark_no), function()
    require('harpoon.mark').set_current_at(mark_no)
  end, { desc = string.format('Set current file to mark %d', mark_no) })
end
for mark_no = 1, 4 do
