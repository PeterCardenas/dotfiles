...
local function bazel_go_lint(abs_filepath)
  ...
  local success, output = shell.async_cmd('cat', { workspace_root .. '/.dev_docker_name' })
  local dev_docker_name ---@type string
  if not success or not output[1] then
    dev_docker_name = 'unknown'
    return
  else
    dev_docker_name = output[1]
  end
  local output_base_flag = string.format('--output_base=$HOME/.cache/bazel/_bazel_go_build_lint_%s', dev_docker_name)
