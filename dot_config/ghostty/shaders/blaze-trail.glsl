...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    newColor = mix(newColor, iCurrentCursorColor, 1.0 - smoothstep(d2, -0.01, 0.001));
    newColor = mix(newColor, iCurrentCursorColor, 1.0 - smoothstep(d2, -0.01, 0.001));
    newColor = mix(newColor, iCurrentCursorColor, antialising(d2));

    float cCursorDistance = sdBox(vu, currentCursor.xy - (currentCursor.zw * offsetFactor), currentCursor.zw * 0.5);
    newColor = mix(newColor, iCurrentCursorColor, 1.0 - smoothstep(cCursorDistance, -0.000, 0.003 * (1. - progress)));
    newColor = mix(newColor, iCurrentCursorColor, 1.0 - smoothstep(cCursorDistance, -0.000, 0.003 * (1. - progress)));

    float pCursorDistance = sdBox(vu, previousCursor.xy - (previousCursor.zw * offsetFactor), previousCursor.zw * 0.5);
    newColor = mix(newColor, iPreviousCursorColor, antialising(pCursorDistance));

    if (progress < 0.99) {
        fragColor = mix(fragColor, newColor, 1 - progress);
    }
}
