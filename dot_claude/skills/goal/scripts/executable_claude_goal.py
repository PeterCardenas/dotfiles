...
def build_logger(name: str, stream: Any) -> logging.Logger:
...
STDOUT_LOGGER = build_logger("claude_goal.stdout", sys.stdout)
STDERR_LOGGER = build_logger("claude_goal.stderr", sys.stderr)
        ...
        if isinstance(workspace_roots, list):
            for root in workspace_roots:
                if isinstance(root, str):
                    sources.append(cwd_session_id(root))
...
def set_goal(
    conn: sqlite3.Connection, sid: str, objective: str, token_budget: int | None
) -> sqlite3.Row:
    ...
    ts = now()
    status = (
        "budget_limited" if token_budget is not None and token_budget <= 0 else "active"
    )
    execute(
        conn,
        """
...
def render_goal(row: sqlite3.Row | None) -> str:
    ...
    if row["token_budget"] is not None:
        parts.append(
            f"- Token budget: {fmt_tokens(row['token_budget'])} (soft budget; Claude Code custom skills do not expose reliable live token counters)"
        )
    return "\n".join(parts)
...
def render_invoke_result(action: str, goal: sqlite3.Row | None, extra: str = "") -> str:
    ...
    if goal and goal["status"] == "active":
        ...
        body.extend([
            "",
            "Claude instructions: The soft budget is exhausted; summarize progress and ask before continuing.",
        ])
    return "\n".join(body)
    ...
    with sqlite_connect() as conn:
        ...
        if command in {"status", "show", "get", "menu"}:
            return render_invoke_result(
                "status", find_goal(conn, candidate_session_ids())
            )
        if command == "pause":
        ...
        if command == "complete":
            return render_invoke_result(
                "complete", update_status(conn, sid, "complete")
            )
...
def main(argv: list[str]) -> int:
                ...
                with sqlite_connect() as conn:
                    emit(
                        render_invoke_result(
                            "set", set_goal(conn, session_id(), objective, budget)
                        )
                    )
        except Exception as exc:
            emit_error(f"goal error: {exc}")
            return 1
        return 0
    ...
    p_invoke = sub.add_parser(
        "invoke",
        help="Process slash-command arguments and print Claude-facing instructions",
    )
    ...
    try:
        if args.cmd == "invoke":
            ...
            with sqlite_connect() as conn:
                emit(
                    render_invoke_result(
                        "pause", update_status(conn, session_id(), "paused")
                    )
                )
            ...
            with sqlite_connect() as conn:
                emit(
                    render_invoke_result(
                        "resume", update_status(conn, session_id(), "active")
                    )
                )
        elif args.cmd == "clear":
            with sqlite_connect() as conn:
                emit(
                    "Goal cleared."
                    if clear_goal(conn, session_id())
                    else "No goal to clear."
                )
        elif args.cmd == "complete":
            with sqlite_connect() as conn:
                emit(
                    render_invoke_result(
                        "complete", update_status(conn, session_id(), "complete")
                    )
                )
        elif args.cmd == "set":
            ...
            with sqlite_connect() as conn:
                ...
                if args.session_id != session_id():
                    emit(render_goal_json(get_goal(conn, args.session_id)))
                else:
                    emit(render_goal_json(find_goal(conn, candidate_session_ids())))
        elif args.cmd == "stop-hook":
            return stop_hook()
        else:
            emit(parser.format_help().rstrip())
            return 2
    except Exception as exc:
        emit_error(f"goal error: {exc}")
        return 1
